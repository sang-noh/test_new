...
class DataFrameReader:
    ...
    def _impute_operators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        df["attributes.number"] = df.apply(
            lambda row: self._divide_phone_number(row["attributes.number"]), axis=1
        )  # divide the numbers
        df["attributes.number.numeric"] = df["attributes.number"].apply(
            self._extract_numeric
        )  # creating the middle numeric value
        return df
    ...
    def _join_table_on_key(
        self, df_calls: pd.DataFrame, df_operators: pd.DataFrame
    ) -> pd.DataFrame:
        """
        Join dataframe and filter based on the values
        """
        joined_dataframe = pd.merge(df_calls, df_operators, how="cross")
        dataframe_after_comparison = joined_dataframe[
            joined_dataframe.apply(self._safe_comparison, axis=1)
        ]
            ...
            "calls_data": pa.DataFrameSchema(
                {
                    ...
                    "operator": pa.Column(
                        ...
                        checks=pa.Check.str_length(
                            min_value=1
                        ),  # non-empty operator names
                    ),
                    "number": pa.Column(
                        str,
                        checks=[
                            pa.Check.str_startswith(
                                "+44", error="Invalid country code"
                            ),  # check UK numbers
                            pa.Check(
                                lambda s: s.fillna(
                                    "withheld"
                                ),  # allow and fill missing values
    ...
    def _total_transformation(self) -> pd.DataFrame:
        ...
        flattened_calls_df["attributes.number"] = flattened_calls_df.apply(
            lambda row: self._divide_phone_number(row["attributes.number"]), axis=1
        )
        flattened_calls_df["attributes.number.numeric"] = flattened_calls_df[
            "attributes.number"
        ].apply(self._extract_numeric)
        joined_dataframe = self._join_table_on_key(
            flattened_calls_df, flattened_operators_df
        )
        ...
        joined_dataframe["date"] = pd.to_datetime(
            joined_dataframe["date"]
        ).dt.date  # get the YYYY-MM-DD
        joined_dataframe["date"] = pd.to_datetime(
            joined_dataframe["date"]
        )  # reassign type to date
        joined_dataframe["score"] = joined_dataframe.apply(
            lambda row: self._assign_risk_score(
                row["greenlist"], row["redlist"], row["riskscore"]
            ),
            axis=1,
        )  # compute score
        joined_dataframe = joined_dataframe[
            ["id", "date", "operator", "number", "score"]
        ]  # get the right columns
        joined_dataframe = joined_dataframe.sort_values(by="date").reset_index(
            drop=True
        )  # reindex
